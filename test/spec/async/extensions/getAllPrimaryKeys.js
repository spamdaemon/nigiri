describe("getAllKeys", function() {
    ...
    async.it("get all primary keys in the index",zone.inject(["#done","nigiri.Nigiri"], function(done,Nigiri) {

        var theStore = setup.db.transaction([ "store" ]).objectStore("store");
        var theIndex = theStore.index("value");
        var req = theIndex.getAllPrimaryKeys(new Nigiri.KeySet([ "a", "c", "e", "g", "i", "k" ]));
    ...
    async.it("get all keys in an object store", zone.inject(["#done","nigiri.Nigiri"], function(done,Nigiri) {
        ...
        var theStore = setup.db.transaction([ "store" ]).objectStore("store");
        var req = theStore.getAllPrimaryKeys(new Nigiri.KeySet([ 0, 2, 4, 6, 8, 10 ]));
